...
def exists(samaccountname):
    searchpath='ou=some,dc=search,dc=base'
    search = 'samaccountname='+str(samaccountname)
    result = ldap_client.search_s(
        searchpath,ldap.SCOPE_SUBTREE,search,['sAMAccountName'])
    if "sAMAccountName" not in result[0][1]:
        return 0
    else:
        return 1
...
def listou(base=None, objectType='samaccountname'):
    if base is None:
        base = self.LDAP_USER_BASE
    search = '%s=*' % (objectType)
    result = ldap_client.search_s(
...
def get_dn_from_sn(samaccountname):
    ...
    search = 'samaccountname='+str(samaccountname)
    result = ldap_client.search_s(
        searchpath,ldap.SCOPE_SUBTREE,search,['distinguishedName'])
    if not len(result):
        return 0
    else:
        return result[0][0] # We need a more reliable way to get this info.
...
# getattr(samaccountname)
#
def old_getattr(samaccountname, attr="*"):
